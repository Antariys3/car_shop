from carshop.models import Car, Order, OrderQuantity


def order_saver(order, client, car):
    if order:
...
def img_finder(order_quantity):
    return order_quantity.car_type.image.url if order_quantity.car_type.image else None
